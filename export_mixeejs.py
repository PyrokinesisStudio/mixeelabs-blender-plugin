...
arrlength=len(ob.data.shape_keys.key_blocks)
curitem=0

for i in range(1,arrlength):
    for j in range(1,arrlength):
        if j==i:
            ob.data.shape_keys.key_blocks[j].value=1.0
        else:
            ob.data.shape_keys.key_blocks[j].value=0.0
        ob.data.shape_keys.key_blocks[j].keyframe_insert("value",frame=i+1)

for shape in ob.data.shape_keys.key_blocks:
    shape.value=0.0
    ob.data.shape_keys.key_blocks[shape.name].keyframe_insert("value",frame=1)
...
"metadata" :
{
	...
	"type"          : "scene",
	"sourceFile"    : "%(fname)s",
	"generatedBy"   : "Blender 2.66 Exporter Mixee Hack",
	"objects"       : %(nobjects)s,
...
def generate_ascii_model(meshes, morphs,
    ...
    if option_animation_morph:
        ...
        for i, morphVertices in enumerate(morphs):
            morphName = bpy.context.object.data.shape_keys.key_blocks[i].name
            morphTarget = '{ "name": "%s", "vertices": [%s] }' % (morphName, morphVertices)
